...
func TestOne(t *testing.T) {
	...
	if *posixServerBinary == "" {
		t.Fatal("--posix_server_binary is missing")
	}
	if *testbenchBinary == "" {
	...
	if *dutPlatform == "netstack" {
		if _, err := dockerutil.RuntimePath(); err != nil {
			t.Fatal("--runtime is missing or invalid with --dut_platform=netstack")
		}
	}
	...
	for _, d := range []struct {
		*dockerutil.Docker
		...
		// TODO(eyalsoha): Remove the "Privileged" below and adjust
		// copies later on to not copy into "/"?
		if err := d.Docker.Create(dockerutil.RunOpts{Image: "packetimpact", Privileged: true, CapAdd: []string{"NET_ADMIN"}, Extra: []string{"--sysctl", "net.ipv6.conf.all.disable_ipv6=0", "--rm", "--stop-timeout", "60", "-it"}}); err != nil {
	...
	cmd1 := testutil.Command(logger("cp"), "docker", "cp", "-L", *posixServerBinary, dut.Name+":"+containerPosixServerBinary)
	cmd1.Stdout = os.Stdout
	cmd1.Stderr = os.Stderr
	if err := cmd1.Run(); err != nil {
		t.Fatal("can't copy posix_server to dut:", err)
	}
	containerTestbenchBinary := "/" + path.Base(*testbenchBinary)
	// TODO(eyalsoha): Convert the below line to testbench.Copy(...)
	cmd2 := testutil.Command(logger("cp"), "docker", "cp", "-L", *testbenchBinary, testbench.Name+":"+containerTestbenchBinary)
	...
	// TODO(igudger): distinguish real errors from EOF and log them.
	...
	// TODO(igudger): distinguish real errors from EOF and log them.
	copyIO(unittestLogger.Logf, ptmx)
